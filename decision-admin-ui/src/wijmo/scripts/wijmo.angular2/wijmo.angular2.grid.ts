...
export module wj.angular2 {
    ...
    */
    ...
    class DirectiveCellFactory extends wijmo.grid.CellFactory {
                ...
                if (col) {
                    ...
                    if (templContext) {
                        ...
                        if (isTpl) {
                            ...
                            let cellInfo = this._setViewRefVars(cellContext.viewRef, row, col, dataItem, cellValue);
                            ...
                            if (isEdit) {
                                ...
                                var editEndingEH = function (s, e) {
                                    ...
                                    if (!e.cancel) {
                                        ...
                                        panel.grid.setCellData(rowIndex, colIndex, cellInfo.value);
                                    }
        ...
        private _setViewRefVars(viewRef: ngCore.EmbeddedViewRef, row: wijmo.grid.Row, col: wijmo.grid.Column,
            dataItem, cellValue): { row: any, col: any, item: any, value: any } {
            viewRef.setLocal('row', row);
            viewRef.setLocal('col', col);
            viewRef.setLocal('item', dataItem);
            //viewRef.setLocal('value', cellValue);
            let cellCtx = { row: row, col: col, item: dataItem, value: cellValue };
            viewRef.setLocal('cell', cellCtx);
            return cellCtx;
        }
    ...
    //    //template: `<ng-content></ng-content>`,
    //})
    //export class WjNestCmp {
    //}


    @Component({
        selector: 'wj-template-cmp',
    })
    @View({
        // we need a div here to supply instantiated templates with a root in shadow DOM
            //template: `<template>Hey: <ng-content></ng-content></template>`,
            template: `<template>Hey: <ng-content></ng-content></template>`,
            //template: `<ng-content></ng-content>`,
            //directives: [WjNestCmp]
    })
    export class WjTemplateCmp  {
        @ContentChildren(TemplateRef) contentChildren: QueryList<TemplateRef>;
        @ViewChildren(TemplateRef) viewChildren: QueryList<TemplateRef>

        constructor( @Inject(ElementRef) elRef: ElementRef,
            @Inject(Injector) injector: Injector,
            @Inject(ViewContainerRef) private viewContainerRef: ViewContainerRef,
            @Inject(TemplateRef) @Optional() private templateRef: TemplateRef,
            @Query(TemplateRef) private templates: QueryList<TemplateRef>
            ) {
        ...
        ngAfterContentInit() {
            var tr = this.templates;
            var cc = this.contentChildren;
            var bbb = true;
        }

        ngAfterViewInit() {
            var self = this;
            var vc = this.viewChildren;
            var tr: TemplateRef = vc.first;
            var viewRef = this.viewContainerRef.createEmbeddedView(tr, 0);
            setTimeout(function () {
